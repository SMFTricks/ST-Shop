...
function template_shop_item_who_owns()
{
	...
	if (empty($context['list_users']))
	{
		echo $context['shop']['who_none'];
	}
	...
	echo '
		<table class="table_grid">
			<thead>
				<tr class="title_bar">
					<th class="centertext first_th">', $txt['Shop_user_avatar'], '</th>
					<th>', $txt['Shop_user_name'], '</th>
					<th class="centertext last_th">', $txt['Shop_user_count'], '</th>
				</tr>
			</thead>
			<tbody>';

		// List the users that have that item
		foreach ($context['list_users'] AS $user)
		{
			echo '
				<tr class="stripes">
					<td class="centertext">', $user['avatar'], '</td>
					<td><a', empty($user['online_color']) ? '' : ' style="color: '. $user['online_color']. '"', ' href="', $scripturl, '?action=profile;u=', $user['id_member'], '">', $user['name'], '</a></td>
					<td class="centertext">', $user['count'], '</td>
				</tr>';
		}
	...
	if (!empty($context['page_index']))
		echo '
		<div class="pagesection" style="clear: none; margin: 0;">
