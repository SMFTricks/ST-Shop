	...
	public static function Trans()
	{
		...
		elseif (!empty($amount) && $_REQUEST['type'] == 'deposit')
		{
			...
			elseif ($deposit >= 0 || $user_info['shopBank'] >= $modSettings['Shop_bank_deposit_fee'])
			{
				...
				parent::logBank($user_info['id'], $amount, $modSettings['Shop_bank_deposit_fee'], $dtype);
		...
		elseif (!empty($amount) && $_REQUEST['type'] == 'withdraw')
		{
			...
			elseif ($withdraw >= 0)
			{
				...
				parent::logBank($user_info['id'], $amount, $modSettings['Shop_bank_withdrawal_fee'], $wtype);
				// Send the user to the next page
