...
function Shop_profileAreas(&$profile_areas)
{
	...
	add_integration_function('integrate_load_profile_fields', 'Shop_profileFields', false);
	add_integration_function('integrate_setup_profile_context', 'Shop_profileSetup', false);
}

function Shop_profileData(&$columns, &$tables, &$set)
{
	switch ($set)
	{
		case 'normal':
...
function Shop_profileContext(&$data, $user, $display_custom_fields)
{
	...
	$profile['shop_inventory'] = Shop_profileInventory($profile);
	$data += array(
...
function Shop_profileCustomFields($memID, $area)
{
...
function Shop_profileFields(&$profile_fields)
{
...
function Shop_profileSetup(&$fields)
{
...
function Shop_profileRegister(&$regOptions, &$theme_vars, &$knownInts, &$knownFloats)
{
	global $modSettings;
