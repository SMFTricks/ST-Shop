...
function Shop_mainGift()
{
	...
	// Sub-menu tabs
	$context['gift_tabs'] = Shop_giftTabs();
...
function Shop_giftSend()
{
	...
	if (!isset($_REQUEST['money']) && isset($_REQUEST['item']))
	{
		...
		else
		{
			...
			Shop_logGift($user_info['id'], $memID, $message, 0, $row['itemid'], $row['id']);
			// Send a PM to the user that its going to receive the item.
			Shop_giftPM('item', $memID, $row['name'], '', $message);
	...
	else
	{
		...
		{
			// Send the gift and log the information
			Shop_logGift($user_info['id'], $memID, $message, $amount);
			// Send a PM to the user that its going to receive the money.
			Shop_giftPM('money', $memID, '', $amount, $message);
			// Let's get out of here and later we'll show a nice message
...
function Shop_giftPM($todo, $userid, $itemname, $amount, $message)
{
